#!/usr/bin/env node
/**
 * Download the default model (and ensure KB is ready) so the app opens without the error screen.
 * Run from desktop/: npm run seed-defaults
 *
 * - Model: downloaded to data/models/ (same path the app uses in dev).
 * - KB: the app will auto-ingest from desktop/test_knowledge_base.json on first run if the
 *   global collection is empty, so no separate download step.
 */
...
async function main() {
  ...
  if (existsSync(modelPath)) {
    console.log('Default model already present at:', modelPath);
  ...
  try {
    await download(DEFAULT_MODEL_URL, modelPath, (received, total) => {
      if (total > 0 && received % (50 * 1024 * 1024) < 1024 * 1024) {
